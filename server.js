...
const connection = mysql.createConnection({
  ...
  port: 3306, // Default MySQL port, verify if this is the right port
  user: 'aad772_gallery',
  password: 'P3t3rs4m', // Ensure this is correct
  database: 'db_aad772_gallery',
...
async function getArtistsByType() {
  return new Promise((resolve, reject) => {
    const query = `
      SELECT ExID, Name, Surname, ProfileImage 
      FROM exhibitions 
    ...
    connection.query(query, (err, results) => {
      ...
      const artists = results.map(row => ({
        ...
        name: row.Name,
        surname: row.Surname,
        profileImage: row.ProfileImage ? row.ProfileImage.toString('base64') : null  // Convert image to base64 if exists
      }));
...
async function bookings(name, email, phone, numberOfPeople, exhibitionName, password) {
  return new Promise((resolve, reject) => {
    ...
    const values = [name, email, phone,  currentDate, numberOfPeople, exhibitionName, password];
    ...
    connection.query(query, values, (err, result) => {
      ...
      resolve({
        ...
        BookingDate: currentDate // Include the booking date in the response
      });
    });
  });
}
...
async function getUserBooking(userId) {
  return new Promise((resolve, reject) => {
    ...
    connection.query(query, [userId], (err, results) => {
      ...
      // Assuming only one result for a specific user
